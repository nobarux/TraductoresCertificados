...
<table width="100%" cellpadding="2" cellspacing="0">
    ...
    <tr>
        <h3>Estimado: {{$sms['nombre']}}</h3>
        {{--        <h3>Estimados</h3>--}}

        <p>Su turno para el día {{ date('d/m/Y', strtotime($sms['fecha'])) }} ha sido creado correctamente. Conserve
            este correo como confirmación
            en caso de ocurrir algún problema con su turno </p>
    </tr>
    <tr>
        <table style="border: 2px; width:100%">
            ...
            <tbody>
            <?php
            $idiomas = \App\Certificacion\Idioma::where('habana', $sms['sede'])->get();

            $tipoDocumentos = \App\TipoDocumento::all();

            foreach ($idiomas as $idioma) {
                foreach ($tipoDocumentos as $tipo) {
                    $index = $idioma->id . "-" . $tipo->id;
                    if ($sms[$index] != 0 && $sms[$index] != '') {
                        echo '<tr>';
                        echo '<td>' . $idioma->nombre . '</td>';
                        echo '<td></td>';
                        echo '<td>' . $tipo->nombre . '</td>';
                        echo '<td></td>';
                        echo '<td>' . $sms[$index] . '</td>';
                        echo '</tr>';
                    }
                }
            }
    ...
    <tr>
        <img
            src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->merge('images/logo mail.png', 0.3, true)->errorCorrection('H')->size(120)->generate("http://siti.esti.cu/Recepcion/turnos/comprobante/" . $sms['id'])) !!} ">
    </tr>
    <tr>
    <!--p>Enviado desde el sitio web: <a href="{{route('inicio')}}">www.esti.cu</a> </p>
        <p>Formulario de contacto del Cliente.</p-->
    </tr>
</table>
